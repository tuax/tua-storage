    ...
    test('batch save items with one key and last one disable cache', () => {
        ...
        return tuaStorage
            ...
            .then(() => {
                ...
                // storage
                expect(store.size).toBe(1)
                expect(JSON.stringify(store.get(targetKey))).toBe(expectedLastVal)
            })
    })
        ...
        return tuaStorage
            ...
            .then(() => {
                ...
                kdArr.map(({ key, data }) => {
                    ...
                    // storage
                    expect(store.size).toBe(kdArr.length)
                    expect(JSON.stringify(store.get(targetKey))).toBe(expectedVal)
                })
            })
    })
        ...
        return tuaStorage
            ...
            .then((loadedItems) => {
                ...
                loadedItems.map(({ code, data: loadedData }, idx) => {
                    ...
                    expect(JSON.stringify(store.get(targetKey))).toBeUndefined()
                })
            })
    })
        ...
        return tuaStorage
            ...
            .then(() => {
                ...
                kdArr.map(({ key, data }) => {
                    ...
                    // storage
                    expect(store.size).toBe(deltaLen)
        ...
        return Promise
            ...
            .then(() => {
                ...
                kdArr.map(({ key, data }) => {
                    ...
                    // storage
                    expect(store.size).toBe(whiteList.length)

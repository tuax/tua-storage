import Storage, { MSG_KEY, DEFAULT_EXPIRES } from '../storage'
import {
    getObjLen,
    expireTime,
    getTargetKey,
...
describe('batch load', () => {
    ...
    test('load some exist items with one key and disable cache', () => {
        ...
        return expect(tuaStorage.save(dataArr).then(() => tuaStorage.load(dataArr)))
        ...
        return tuaStorage
            ...
            .then((loadedItems) => {
                ...
                loadedItems.map(({ code, data: loadedData }, idx) => {
                    ...
                    const expectedVal = getExpectedValBySyncFn(data, DEFAULT_EXPIRES)
    ...
    test('save many items with one key', () => {
        ...
        return Promise
            ...
            .then(() => {
                ...
                const targetKey = getTargetKey(key)
                // last value
                const expectedVal = getExpectedVal(dataArr.slice().pop(), DEFAULT_EXPIRES)
...
describe('load', () => {
    ...
    test('load one exist item without cache', () => {
        ...
        return expect(tuaStorage.save({ key, data })
